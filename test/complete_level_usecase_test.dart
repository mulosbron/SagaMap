...
void main() {
  group('CompleteLevelUseCase', () {
    ...
    test('the 1.x boss placement can be restored with bossRule', () {
      ...
      expect(
        legacy
            .execute(currentProgress: _reached(15), levelId: 15, globalSeed: 99)
            .reward,
        isNotNull,
      );
      expect(
        legacy
            .execute(currentProgress: _reached(14), levelId: 14, globalSeed: 99)
            .reward,
        isNull,
      );
      expect(
        legacy
            .execute(currentProgress: _reached(0), levelId: 0, globalSeed: 99)
            .reward,
        isNull,
      );
    });
  ...
  group('CompleteLevelUseCase injection', () {
    ...
    test('a custom bossRule decides which levels drop', () {
      ...
      expect(
        useCase
            .execute(currentProgress: _reached(7), levelId: 7, globalSeed: 1)
            .reward,
        isNotNull,
      );
      expect(
        useCase
            .execute(currentProgress: _reached(8), levelId: 8, globalSeed: 1)
            .reward,
        isNull,
      );
      expect(
        useCase
            .execute(currentProgress: _reached(15), levelId: 15, globalSeed: 1)
            .reward,
        isNull,
      );
    });
  ...
  group('the guards that ship by default', () {
    test('enforceUnlockOrder is on out of the box', () {
      // T-11: it used to be an execute parameter defaulting to false, so the
      // shipped configuration accepted any level id and the guard was
      // something a host had to remember at five call sites.
      const useCase = CompleteLevelUseCase();
      expect(useCase.enforceUnlockOrder, isTrue);
      ...
      expect(result.nextProgress.levels.containsKey(500), isFalse);
      expect(result.reward, isNull);
    });

    test('a call may still opt out for one jump', () {
      const useCase = CompleteLevelUseCase();
      ...
      final result = useCase.execute(
        ...
        levelId: 500,
        globalSeed: 42,
        enforceUnlockOrder: false,
      );

      expect(result.outcome, CompleteLevelOutcome.applied);
      expect(
        result.nextProgress.levels[500]?.state,
        LevelCompletionState.completed,
      );
    });

    test('an instance may be built with the guard off', () {
      const useCase = CompleteLevelUseCase(enforceUnlockOrder: false);
    ...
    test('a negative level is refused whatever the guard says', () {
      const useCase = CompleteLevelUseCase(enforceUnlockOrder: false);
      ...
      expect(result.outcome, CompleteLevelOutcome.rejectedUnreached);
      expect(result.nextProgress.levels.containsKey(-5), isFalse);
    });
  });

  group('the sharp edges that remain', () {
    test('rollBossReward mints twice when called directly', () {
      // T-11.05: pinned, not fixed. rollBossReward is a public, unguarded mint
      // and its doc comment now says so in full. This test exists so the edge
      // cannot quietly get sharper.
      final first = rollBossReward(levelId: 14, globalSeed: 7);
      final second = rollBossReward(levelId: 14, globalSeed: 7);

      expect(first.itemId, second.itemId);
      expect(first.obtainedFromLevelId, second.obtainedFromLevelId);
    });
    ...
    test('two execute calls on one snapshot both mint', () {
      ...
      expect(a.reward, isNotNull);
      expect(b.reward?.itemId, a.reward?.itemId);

      // Threading the first result into the second closes it.
      final c = useCase.execute(
        currentProgress: a.nextProgress,
        levelId: 14,
        globalSeed: 7,
      );
...
/// Progress in which [levelId] has actually been reached, so the default
/// `enforceUnlockOrder` guard lets it be completed. These tests are about what
/// a completion *rewards*, not about progression order.
SagaProgress _reached(int levelId) {
  return SagaProgress(
