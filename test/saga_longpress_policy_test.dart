  ...
  group('Saga long press gesture', () {
    Widget buildAdapter({
      ...
      final renderContext = SagaMapRenderContext(
        layout: const ResolvedSagaLayout(breakpoint: SagaMapBreakpointName.desktop, pathAxis: SagaMapPathAxis.vertical, nodeSize: 1, nodeSpacing: 1, zoom: 1, interactionRadius: 1.0, cameraPadding: 0, scrollSensitivity: 1, maxLateralExtent: 100),
        levels: [const LevelData(id: 0, position: SagaPoint(0, 0), biomeId: 'test')],
        progressResolver: (l) => LevelProgress(levelId: 0, state: state, stars: 1),
          ...
          body: Builder(
            builder: (context) {
              final adapter = WidgetRendererAdapter(
                context: context,
                nodeBuilder: (context, level, layout) => const SizedBox(width: 44, height: 44),
    ...
    testWidgets('emitTapForLockedNode: false + locked node -> no long press callback', (tester) async {
    ...
    testWidgets('emitTapForLockedNode: true + locked node -> callback fires', (tester) async {
      int longPressCount = 0;
    ...
    testWidgets('emitTapForCompletedNode: false + completed node -> no callback', (tester) async {
      int longPressCount = 0;
    ...
    testWidgets('custom policy can allow long press even if tap is false', (tester) async {
      int longPressCount = 0;
